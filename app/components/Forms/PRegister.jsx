    ...
    try {
      ...
      if (image) {
        ...
        await setDoc(doc(db, "doctors", user.uid), {
          ...
          email: data.email,
          createdAt: new Date(),
          profilePicture: downloadURL, // Add the profile picture URL to the document
        });
      } else {
        // If no image, save the user's data without the profile picture
        await setDoc(doc(db, "doctors", user.uid), {
          name: data.name,
          email: data.email,
          createdAt: new Date(),
        ...
        <div className="flex items-center justify-center w-full lg:w-1/2 pt-[2%]">
          <div className="w-[28rem] p-8 bg-white rounded-[15px] shadow-lg h-full">
            ...
            <div className="flex items-center justify-center gap-2 my-6 text-center">
              ...
              <Link href="/login" className="text-violet-600">Click Here</Link>
            </div>
            <Toaster />
          </div>
        </div>
